...
public class SignalResource {
  ...
  @ApiImplicitParams({
    ...
    @ApiImplicitParam(name="tenantId", dataType="", value="ID of the tenant that the signal event should be processes in"),
  ...
  @ApiResponses(value = {
      @ApiResponse(code = 200, message = "Indicated signal has been processes and no errors occurred."),
      @ApiResponse(code = 202, message = "Indicated signal processing is queued as a job, ready to be executed."),
      @ApiResponse(code = 400, message = "Signal not processes. The signal name is missing or variables are used together with async, which is not allowed. Response body contains additional information about the error.")
  })
